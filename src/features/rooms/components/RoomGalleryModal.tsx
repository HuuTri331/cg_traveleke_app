...
import { Star, Trash2, CheckCircle2, Image as ImageIcon, AlertCircle } from 'lucide-react';
...
export function RoomGalleryModal({
  ...
  }, [isOpen, room, fetchImages]);

  const handleUpload = async (files: File[]) => {
    if (!room) return;
    try {
      setUploading(true);
      await roomsApi.uploadImages(room.id, files);
      success('Tải ảnh thành công', `Đã thêm ${files.length} ảnh vào album phòng!`);
      await fetchImages();
      onSuccess();
    } catch (err: any) {
  ...
  const handleSetPrimary = async (imageId: string) => {
    ...
    try {
      ...
      success('Đã đổi ảnh đại diện', 'Ảnh được chọn đã được đặt làm ảnh đại diện cho phòng.');
      await fetchImages();
      onSuccess();
    } catch (err: any) {
  ...
  const handleDelete = async (imageId: string) => {
    ...
    try {
      ...
      success('Đã xóa ảnh', 'Ảnh đã được gỡ khỏi album phòng thành công.');
      await fetchImages();
      onSuccess();
    } catch (err: any) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Album Ảnh: ${room.name}`}
      subtitle={`Quản lý tối đa 5 ảnh cho phòng nghỉ. Hiện có ${images.length}/5 ảnh.`}
      maxWidth="4xl"
    >
      <div className="space-y-6">
        ...
        {images.length < 5 ? (
          <div className="rounded-2xl bg-gray-50/70 p-5 dark:bg-gray-850/50 border border-gray-200 dark:border-gray-800">
            <h4 className="text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-300 mb-3 flex items-center gap-2">
              <ImageIcon className="h-4 w-4 text-brand-500" /> Tải thêm nhiều ảnh phòng (Tối đa 5 ảnh)
            </h4>
            <ImageUploadZone
              maxFiles={5}
          ...
          <div className="flex items-center gap-2 rounded-xl bg-amber-50 p-4 text-xs font-medium text-amber-800 dark:bg-amber-950/30 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
            ...
            Phòng nghỉ này đã đạt giới hạn tối đa 5 ảnh trong album. Hãy xóa bớt ảnh cũ nếu bạn muốn thêm ảnh mới.
          </div>
        )}
        ...
        <div>
          <h4 className="text-xs font-bold uppercase tracking-wider text-gray-700 dark:text-gray-300 mb-3 flex items-center justify-between">
            <span>Danh sách ảnh phòng hiện tại ({images.length})</span>
          </h4>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-300 py-12 text-center dark:border-gray-700">
              ...
              <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                Chưa có ảnh nào trong album phòng
              </p>
              <p className="text-xs text-gray-500">
                Hãy kéo thả hoặc chọn file ở khung trên để thêm ảnh trực quan.
              </p>
            </div>
          ) : (
